...
puts "Le handle le plus petit est #{minHandle}"


handle5Element =  arr.find_all{|m| m.length == 6}


print  handle5Element




handleCapi = arr.find_all{|a| a[1] == a[1].capitalize }

puts ""
puts handleCapi.length
...
#Trier la liste de handle par taille des handle (les plus petits en premiers, les plus grands après
...
puts "La position dans l'array de la personne @epenser est  #{arr.index("@epenser")}"

arr.index("@epenser")



# Répartiotion de handle par taille

arrNb = []

for i in 2..arr.max.length

    arrNb << {"#{i-1} caracteres" => arr.find_all{|m| m.length == i}.length}
